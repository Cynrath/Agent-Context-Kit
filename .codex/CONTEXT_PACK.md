...
- TASK-0123 local validation passed with 186/186 tests, package inspection, full installed-tool smoke, clean source outputs, all local gates, and 3.704/3.685-second performance evidence. Commits `63ef69c`, `6289acb`, and `4f5f06c` each passed 8/8; their release dispatches stopped before publication while exposing Markdown-link host/path behavior. The final fix uses isolated `pwsh` and repo-relative segment normalization with real-escape tests; a new exact SHA and fresh 8/8 are required.
