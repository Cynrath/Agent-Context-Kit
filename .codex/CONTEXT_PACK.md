...
- TASK-0123 local validation passed with 186/186 tests, package inspection, full installed-tool smoke, clean source outputs, all local gates, and 3.704/3.685-second performance evidence. Commits `63ef69c` and `6289acb` each passed 8/8; both release dispatches stopped before publication on Markdown-link host behavior. The workflow now invokes these gates in isolated `pwsh` child processes and preserves fixture failure output; a new exact SHA and fresh 8/8 are required.
