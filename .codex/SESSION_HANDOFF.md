...
- TASK-0123 commits through `ed9bf78` each passed 8/8 standard hosted jobs. The latest release dispatch completed validation/package upload, then stopped before OIDC on Ubuntu because release-job scripts used `powershell`. Those calls now use cross-platform `pwsh`; a new exact release commit and fresh 8/8 are required.
