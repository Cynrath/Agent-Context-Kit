...
- TASK-0123 candidate commit `63ef69c` passed 8/8 standard hosted jobs. The first manual release dispatch stopped before publication because Windows PowerShell 5.1 lacked one direct `Path.GetRelativePath` call in the link checker; the URI-based compatibility fix and repository-escape regression case are being committed as a replacement exact release candidate.
