...
| 22 | Done locally | TASK-0087 JSON/SARIF/report baseline integration | High | Hosted/package evidence remains separate | output contracts/tests/docs | Full local gates | No | Additive baseline metadata is consistent across machine/local reports. |
...
| 27 | Done locally | TASK-0092 release-candidate contract freeze and maintainer decision | High | Hosted/remote P0/P1 evidence remains | contract/release/readiness docs | Full local gates plus maintainer-hosted evidence | Yes, for hosted/release actions | Local contract freeze and maintainer GO/NO-GO conditions are explicit without premature approval. |
| 28 | Next | TASK-0093 machine-readable JSON schema and golden contract assets | High | V100-04 remains P0 | schema assets/tests/docs | Full contract tests and local gates | No | Versioned machine-readable schemas/golden fixtures enforce additive and breaking-change rules. |
| 29 | Queued | TASK-0094 English/Turkish localization parity release gate | High | V100-10 remains open | CLI/tests/docs/scripts | Full localized contract matrix | No | Help, diagnostics, errors, and JSON invariance are release-gated. |
| 30 | Queued | TASK-0095 security reporting and supply-chain maintainer evidence handoff | High | Remote settings and publication decisions | security/supply-chain/release docs | Local checklist plus maintainer evidence | Yes | Private reporting and signing/SBOM/provenance decisions have dated maintainer evidence. |
