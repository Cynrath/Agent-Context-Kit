...
| V100-09 | P1 | Release supply-chain policy still lacks dated maintainer decisions and aligned NuGet ownership identity. | TASK-0099 verified a NuGet.org repository signature, no observed author signature, no package/release SBOM, no accessible GitHub package attestation, and owner profile `Cyranth` versus project persona `Cynrath`. | Release + Maintainer | Align or accept the owner identity; record owned signing, SBOM, provenance, recovery, and residual-risk decisions; perform only approved remote actions. | Yes | Yes, for ownership/signing/provenance publication |
