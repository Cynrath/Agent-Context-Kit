...
| --- | --- | --- | --- | --- | --- | --- | --- |
| V100-01 | P0 | Baseline-aware CI policy is not implemented. | TASK-0084 adds the sanitized model and fingerprint foundation; file workflow, scan classification, and CI policy remain open. | Core + CLI | Versioned baseline serialization, explicit update flow, new-versus-existing tests, Critical safety tests, and cross-platform package evidence. | Yes | No |
...
| V100-03 | P0 | Config schema accepts unknown keys and has no validation/migration contract. | Config schema `1`, convention docs, allowlist tests. | Core + CLI | Deterministic config diagnostics, unknown/obsolete key policy, schema migration rules, invalid-config exit behavior, tests and docs. | Yes | No |
